...
boolean EasyTransfer::receiveData(){
  ...
  //this size check may be redundant due to the size check below, but for now I'll leave it the way it is.
	...
	//this will block until a 0x06 is found or buffer size becomes less then 3.
      ...
      if (_serial->read() == 0x85){
			// get message type
			mt = message_types[_serial->read()];
			mt.rx_len = _serial->read();
			//make sure the binary structs on both Arduinos are the same size.
			if(mt.rx_len != mt.size){
			  mt.rx_len = 0;
			  return false;
			}
      }
  ...
  if(mt.rx_len != 0){
    while(_serial->available() && mt.rx_array_inx <= mt.rx_len){
      mt.rx_buffer[mt.rx_array_inx++] = _serial->read();
    }
    
    if(mt.rx_len == (mt.rx_array_inx-1)){
      //seem to have got whole message
      //last uint8_t is CS
      calc_CS = mt.rx_len;
      for (int i = 0; i<mt.rx_len; i++){
        calc_CS^=mt.rx_buffer[i];
      } 
      
      if(calc_CS == mt.rx_buffer[mt.rx_array_inx-1]){//CS good
        memcpy(address,mt.rx_buffer,mt.size);
		mt.rx_len = 0;
		mt.rx_array_inx = 0;
		return true;
		}
